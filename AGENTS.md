...
- `docs/popular_entity_pools.md`: 인기 감독/배우/제작사/배급사 pool과 파생 feature 생성 규칙
...
- 인기 엔티티 pool/feature 공통 로직은 `src/utils/entity_pool.py`에 둡니다.
- 모델링 코드를 추가할 때도 데이터 생성 절차와 모델 학습 절차를 분리합니다.

## 인기 엔티티 Pool 주의점

감독, 배우, 제작사, 배급사 기반 popular feature는 아래 파일과 스크립트를 사용합니다.

```text
docs/company_alias_map_utf8_sig.csv
src/scripts/build_popular_entity_pools.py
src/scripts/add_popular_entity_features.py
src/utils/entity_pool.py
```

`docs/company_alias_map_utf8_sig.csv`는 사람이 관리하는 제작사/배급사 alias map입니다. `data/processed` 아래 산출물과 달리 Git에 포함해야 합니다.

제작사/배급사는 `entity` 표시명이 아니라 `entity_key` 기준으로 매칭합니다.

alias map을 수정하면 아래 순서로 다시 실행합니다.
...
```text
data/processed/entity_pools/popular_*_pool_utf8_sig.csv
